...
library PairOracleTWAP {
    ...
    function currentTWAP(PairOracle storage self) internal view returns (uint256) {
        ...
        (uint256 price0Cumulative, uint256 price1Cumulative, ) = UniswapV2OracleLibrary.currentCumulativePrices(address(self.pair));
        ...
        unchecked {
            ...
            FixedPoint.uq112x112 memory priceAverage = FixedPoint.uq112x112(
                uint224(((self.token == self.pair.token0() ? price0Cumulative : price1Cumulative) - self.priceCumulativeLast) / timeElapsed)
            );
    ...
    function update(PairOracle storage self) internal {
        (uint256 price0Cumulative, uint256 price1Cumulative, uint256 blockTimestamp) = UniswapV2OracleLibrary.currentCumulativePrices(address(self.pair));
        ...
        unchecked {
            ...
            self.priceAverage = FixedPoint.uq112x112(uint224((newPriceCumulativeLast - self.priceCumulativeLast) / timeElapsed));
